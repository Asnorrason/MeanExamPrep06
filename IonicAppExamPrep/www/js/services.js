angular.module('starter.services', [])
  ...
  .factory("Login", function ($firebaseArray) {
    return {
      login: function (user) {
        ...
        var account = new Firebase("https://ionicchattestapp.firebaseio.com/accounts/" + user.uid);
        if (account == null) {
          account.set(userData);
        }

        //add user to online userlist
        var loginUser = new Firebase("https://ionicchattestapp.firebaseio.com/online/" + user.uid);
        loginUser.set(userData);
      },
      logout: function (user) {
        //Remove user from online userlist
        var removeUser = new Firebase("https://ionicchattestapp.firebaseio.com/online/" + user.uid);
        removeUser.remove();

        //Unathenticate user and remove session token
        var ref = new Firebase("https://ionicchattestapp.firebaseio.com");
        ref.unauth();
      },
      chats: function (user) {
        var chats = new Firebase("https://ionicchattestapp.firebaseio.com/accounts/" + user.uid + "/chats");
  ...
  .factory("Users", function ($firebaseArray) {
    var onlineUsers = new Firebase("https://ionicchattestapp.firebaseio.com/online");
